...
```{r, echo=TRUE}
# decision between marginalized vs. conditionized testing
data.cor.rel[Var1 == "type" | Var2 == "type"]#influence on discount
```

```{r, echo=TRUE, results="hide", message=FALSE}
#subset types
cormat.test.t1 <- cormat.test[type == 1]
cormat.test.t2 <- cormat.test[type == 2]
cormat.test.t3 <- cormat.test[type == 3]
cormat.test.t4 <- cormat.test[type == 4]
cormat.test.t5 <- cormat.test[type == 5]
cormat.test.t6 <- cormat.test[type == 6]
```
...
for (i in 1:6) {
  for (j in 1:6) {
    ...
    component1 <- get(paste0("cormat.test.t", i))
    component2 <- get(paste0("cormat.test.t", j))
...
cormat.res <- cormat.res[component1 != component2]
print(cormat.res)
```
...
```{r, echo=TRUE, results="hide", message=FALSE}
#subset discounts
cormat.test.lam1 <- cormat.test[true_par_lambda == 1]
cormat.test.lam5 <- cormat.test[true_par_lambda == 5]
```

```{r, echo=TRUE}
#correlation test
r.test(n = nrow(cormat.test.lam1),
       n2 = nrow(cormat.test.lam5),
       r12 = cor(cormat.test.lam1$par_tau,
                 cormat.test.lam1$true_par_tau,
                 method = "spearman"),
       r34 = cor(cormat.test.lam5$par_tau,
                 cormat.test.lam5$true_par_tau,
                 method = "spearman"))
```
