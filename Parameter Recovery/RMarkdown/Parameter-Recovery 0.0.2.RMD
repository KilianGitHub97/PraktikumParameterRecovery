...
data.w.mlr$Znblock <- scale(data.w.mlr$nblock)
data.w.mlr$Ztype <- scale(data.w.mlr$type)
data.w.mlr$Zlambda <- scale(data.w.mlr$par_lambda)
data.w.mlr$Ztrue_tau <- scale(data.w.mlr$true_par_tau)
data.w.mlr$Ztau <- scale(data.w.mlr$par_tau)
```

```{r, echo=TRUE}
output.mlr <- lm(formula = Ztau ~ Ztrue_tau + Zlambda + Ztype + Znblock + Zdiscount,
                 data = data.w.mlr)
...
ggplot(data = results.w,
  ...
  geom_line(data = results.w[true_par_discount == 0],
            aes(x = true_par_tau,
                y = par_tau),
            color = "green") +
  geom_line(data = results.w[true_par_discount == 3],
            aes(x = true_par_tau,
                y = par_tau),
            color = "blue") +
  geom_line(data = results.w[true_par_discount == 8],
            aes(x = true_par_tau,
                y = par_tau),
            color = "yellow") +
  ylim(0,10) +
